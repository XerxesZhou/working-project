<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CustomerReceiptList.aspx.cs"
    Inherits="SmartSoft.MobileWeb.CustomerReceiptList" %>
...
<head>
 ...
 <script type="text/javascript">
     function ViewCustomerBusiness(id) {
         window.location.href = "CustomerBusinessEditForm.aspx?&showPanel=divReceiptList&ctr=Receipt&Action=View&ID=" + id;
     }

     FunctionName = "GetCustomerReceiptData";

     function getParameterStr(mode) {

         var keyWord = $("#txtKeyWord").val();
         var crDepartmentID = $("#ddlDepartment").val();
         var crOperatorID = $("#ddlOperator").val();
         var crDateBegin = $("#txtDateBegin").val();
         var crDateEnd = $("#txtDateEnd").val();
         var count = parseInt($("#hfCurrentCount").val());

         var EachCount = parseInt($("#hfEachCount").val());

         var para = "{count:'" + count + "',EachCount:'" + EachCount + "',KeyWord:'" + keyWord + "',crDepartmentID:'" + crDepartmentID + "',crOperatorID:'" + crOperatorID + "',crDateBegin:'" + crDateBegin + "',crDateEnd:'" + crDateEnd + "',CurrentOperatorID:'" + $("#hfCurrentOperatorID").val() + "',cusColumn:'" + $("#ddlTableColumn").val() + "',orderby:'" + $("#ddlOrder").val() + "',mode:'" + mode + "'}";
     ...
     function getLiContent(item) {
         var li = "";
         li += '<li><a class="ui-btn ui-corner-all ui-shadow ui-btn-inline" onclick="ViewCustomerBusiness(' + item.crBusinessID + ')">';
         li += '<h3>';
         li += '    ' + item.cusName + '-' + item.cbName + '</h3>';
         li += '<div class="GroupDiv">';
         li += '<div style="float: left; width: 100%;">';
         li += '        <div class="dis">';
         li += item.crAmount + '</div>';
         li += '</div>';
         li += '</div>';
         li += '<div class="GroupDiv">';
         li += '<div class="ListLeft">';
         li += ' <div class="dis">';
         li += item.crOperatorName + '|' + item.crDate + '</div>';
         li += '    </div>';
         li += '</div>';
         li += '</a>';
         li += '</li>';
                    ...
                    <tr>
                        ...
                        <td>
                            <asp:DropDownList runat="server" data-role="none" ID="ddlTableColumn">
                                ...
                                <asp:ListItem Text="录入时间" Value="crAddDate" Selected="True"></asp:ListItem>
                                <asp:ListItem Text="类型" Value="crTypeID"></asp:ListItem>
                                <asp:ListItem Text="负责人" Value="crOperatorID"></asp:ListItem>
                                <asp:ListItem Text="录入人" Value="crAddOperatorID"></asp:ListItem>
                            </asp:DropDownList>
                        </td>
                    </tr>
                    <tr>
